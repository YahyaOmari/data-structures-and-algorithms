...
const count = (target, input) => {
  // Solution code here...
  let count = 0;
  return input.reduce((acc, element) => acc + element.filter(num => num == target).length,0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let allElements = input.length;
  let diveFive = [];

  for (let i = 0; i < allElements; i++) {
    if (allElements[i] %5 === 0) {
      diveFive.push(allElements[i]);
    }
  }
...
let findShortest = (data) => {
  // Solution code here...
  data.sort((first, second) => first.height - second.height);
  return data[0].name;
};
